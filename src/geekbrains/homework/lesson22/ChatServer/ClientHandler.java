...
public class ClientHandler {
    ...
    public ClientHandler(Socket socket, SuperServer superServer) {
        try {
            ...
            LOGGER.info("Установлено соединение с пользователем");
        ...
        try {
            while (true) {
                ...
                switch (dto.getMessageType()) {
                    ...
                    case PUBLIC_MESSAGE ->
                            {superServer.broadcastMessage(dto);
                            LOGGER.info("От пользователя " + user + " отправлено сообщение в общий чат");
                            }
                    case PRIVATE_MESSAGE -> {
                        superServer.sendPrivate(dto);
                        LOGGER.info("От пользователя " + user + " отправлено приватное сообщение " + dto.getTo());
                    }
...
//                        superServer.changeUserName(dto);
                        ...
                        LOGGER.info("Пользователь " + oldName.getFrom() + " изменил имя на " + dto.getUsername());
                    }
                }
            }
        } catch (SocketException e) {
            LOGGER.info("Соединение с пользователем " + getUser() + " разорвано");
//            System.out.println("Соединение с пользователем " + getUser() + " разорвано");
    ...
    private synchronized void authenticate() {
        LOGGER.info("Пожалуйста авторизуйтесь!");
...
//                String username = superServer.getAuthService().getUsernameAndPass(dto.getLogin(), dto.getPassword());
                ...
                if (!(username == null)) {
                    if (superServer.isUserAuth(username)) {
                        ...
                        sendMessage(send);
                        LOGGER.error("Такой пользователь уже в сети!");
...
//            e.printStackTrace();
            LOGGER.error("Соединение с пользователем разорвано");
        }
    }
